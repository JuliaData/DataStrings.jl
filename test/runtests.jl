...
using Test, Random
using DataStrings
using DataStrings: StringVector, StringPayload, DataBytes, BytesVector
...
function asfrombytes(bytes::Vector{UInt8})
    n = length(bytes)
    n <= AS.INLINE_MAX && return DataString(AS.inline_payload(bytes, 1, n), AS.EMPTY_BYTES)
    return DataString(AS.view_payload(bytes, 1, n, 0, 0), bytes)
end

function asscratchbytes(s::DataString)
...
function sumncodeunits(c::StringVector{DataString})
    t = 0
...
allocated_hash(v::Vector{DataString}) = @allocated(foldashash(v, UInt(9)))
allocated_cmp(v::Vector{DataString}) = @allocated(foldascmp(v))
allocated_access(c::StringVector{DataString}) = @allocated(sumncodeunits(c))
...
function column(strings::Vector; inextra=i -> false)
    ...
    ELT = any(ismissing, strings) ? Union{Missing,DataString} : DataString
    return StringVector{ELT}(payloads, buf, extra)
end

@testset "DataStrings" begin
    ...
    @testset "DataString: equality, hashing, ordering agree with String" begin
        # inline/view boundary: 12 bytes inline, 13 views the buffer
        col = column(["x"^12, "y"^13])
        @test col isa StringVector{DataString}
        ...
        for n = 0:255
            ...
            if n <= AS.INLINE_MAX
                push!(payloads, DataString(AS.inline_payload(bytes, 1, n), AS.EMPTY_BYTES))
            else
                data = vcat(UInt8[0x11], bytes, UInt8[0x22])
                bufidx = isodd(n) ? 1 : 0
                push!(payloads, DataString(AS.view_payload(data, 2, n, bufidx, 1), data))
            end
        end
        ...
        @test promote_type(DataString, String) === String
        io = IOBuffer()
        print(io, col[2])
    ...
    @testset "DataString: iteration and character indexing match String" begin
    ...
    @testset "StringVector: buffers, missing, materialize, allocation" begin
        ...
        @test col isa StringVector{DataString}
        @test length(col) == 1000 && length(col.buffers) == 3
        ...
        @test mcol isa StringVector{Union{Missing,DataString}}
        ...
        @test eltype(mcol) === Union{Missing,DataString}
        ...
        ncol = StringVector{DataString}(payloads, Vector{UInt8}[b0, b1, b2])
        ...
        @test_throws ArgumentError StringVector{DataString}(
        ...
        @test_throws ArgumentError StringVector{Int}(StringPayload[], Vector{UInt8}[])
        badmissing = StringPayload(UInt64(0xfffffffe), zero(UInt64))
        @test_throws ArgumentError StringVector{Union{Missing,DataString}}(
            [badmissing],
            Vector{UInt8}[],
        )
        badprefix = StringPayload(payloads[1].a ⊻ (UInt64(1) << 32), payloads[1].b)
        @test_throws ArgumentError StringVector{DataString}([badprefix], Vector{UInt8}[b0])
        short = AS.inline_payload(b1, 1, 5)
        badpadding = StringPayload(short.a, short.b | (UInt64(1) << 8))
        @test_throws ArgumentError DataString(badpadding, AS.EMPTY_BYTES)
        @test_throws ArgumentError DataString(AS.view_payload(b0, 3, 18, 0, 100), b0)
        # rebase: appending one buffer to another re-points its entries
        combined = vcat(b0, b1)
        rebased = AS.rebase_payload(payloads[2], length(b0))
        @test String(DataString(rebased, combined)) == "second-buffer-value"
    end
    ...
    @testset "StringVector: trusted constructor" begin
        ...
        checked = StringVector{DataString}(payloads, Vector{UInt8}[data])
        trusted = StringVector{DataString}(payloads, Vector{UInt8}[data], Val(:trusted))
        ...
        tv = StringVector{DataString}([badpadding], Vector{UInt8}[data], Val(:trusted))
        @test tv isa StringVector{DataString} && length(tv) == 1
        mp = [AS.PAYLOAD_MISSING]
        mv = StringVector{Union{Missing,DataString}}(mp, Vector{UInt8}[], Val(:trusted))
        @test mv[1] === missing
    end

    @testset "DataBytes and BytesVector" begin
        ...
        col = BytesVector{DataBytes}(payloads, Vector{UInt8}[data])
        @test col isa BytesVector{DataBytes} && length(col) == length(vals)
        ...
        # DataBytes work as Dict keys next to byte vectors
        ...
        ab1 = DataBytes(AS.view_payload(long1, 1, 17, 0, 0), long1)
        ab2 = DataBytes(AS.view_payload(long2, 1, 17, 0, 0), long2)
        @test ab1 != ab2
        @test ab1 == DataBytes(AS.view_payload(long1, 1, 17, 0, 0), long1)

        # missing markers, materialize, and the trusted constructor
        mp = StringPayload[payloads[4], AS.PAYLOAD_MISSING]
        mcol = BytesVector{Union{Missing,DataBytes}}(mp, Vector{UInt8}[data])
        ...
        tcol = BytesVector{Union{Missing,DataBytes}}(mp, Vector{UInt8}[data], Val(:trusted))
        @test tcol[2] === missing && tcol[1] == vals[4]
        # checked constructors reject bad geometry exactly like the string side
        @test_throws ArgumentError BytesVector{DataBytes}(
            [AS.view_payload(data, 1, 20, 0, 100)],
            Vector{UInt8}[data],
        )
        @test_throws ArgumentError BytesVector{Int}(StringPayload[], Vector{UInt8}[])
        @test_throws ArgumentError DataBytes(AS.view_payload(data, 1, 20, 0, 100), data)
    end
end
